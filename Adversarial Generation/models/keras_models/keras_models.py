...
def keras_vgg16_imagenet_model(logits=False, input_range_type=1):
    """
    Run the prediction network *without softmax*.
    """
    input_shape = (224, 224, 3)
    model = VGG16(include_top=True, weights='imagenet', input_tensor=None, input_shape=input_shape, pooling=None, classes=1000, logits=logits, input_range_type=input_range_type)
    return model
