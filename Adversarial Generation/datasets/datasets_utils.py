...
def calculate_mean_distance(X1, X2):
    ...
    mean_l1_dist = np.mean([ np.sum(np.abs(X1[i]-X2[i])) for i in range(len(X1))])
    ...
    return mean_l2_dist, mean_li_dist, mean_l0_dist_value, mean_l0_dist_pixel,mean_l1_dist
...
def evaluate_adversarial_examples(X_test, Y_test, X_test_adv, Y_test_target, targeted, Y_test_adv_pred):
    ...
    mean_l2_dist, mean_li_dist, mean_l0_dist_value, mean_l0_dist_pixel,mean_l1_dist = calculate_mean_distance(X_test[success_idx], X_test_adv[success_idx])
    ...
    print ("L2 dist: %.4f, Li dist: %.4f, L0 dist_value: %.1f%%, L0 dist_pixel: %.1f%%, L1 dist:  %.4f" % (mean_l2_dist, mean_li_dist, mean_l0_dist_value*100, mean_l0_dist_pixel*100,mean_l1_dist))
