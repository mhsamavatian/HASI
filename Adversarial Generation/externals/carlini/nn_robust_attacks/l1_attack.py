        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                if iteration%(self.MAX_ITERATIONS//100) == 0:
                    print(iteration,self.sess.run((self.loss,self.loss2,self.loss21,self.loss2_y,self.loss21_y)))#,self.l2dist,self.l1dist)))
...
class EADL1_adaptive(EADL1):
    def __init__(self, sess, model, full_model, batch_size=1, confidence = CONFIDENCE,
        ...
        self.pool_output_slack = full_model.predict(self.slack)
        ...
        self.l1dist = tf.reduce_sum(tf.abs(self.prob_output-self.pool_output),1)
        self.l1dist_y = tf.reduce_sum(tf.abs(self.pool_output_slack-self.pool_output),1)
    ...
    def attack_batch(self, imgs,imgs_pool, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                if iteration%(self.MAX_ITERATIONS//100) == 0:
                    print iteration,self.sess.run((self.loss,self.loss2,self.loss21))
...
class EADEN_adaptive(EADL1_adaptive):
