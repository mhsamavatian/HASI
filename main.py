...
def main(argv=None):
    # 0. Select a dataset.
    from datasets import MNISTDataset, ImageNetDataset, SVHNDataset, CIFAR10Dataset
    ...
    if FLAGS.dataset_name == 'ImageNet':
            ...
            if FLAGS.model_name == 'vgg16':
                if not (path.exists('image_dataset/X_test_all_imagenet_VGG16') and path.exists('image_dataset/Y_test_all_imagenet_VGG16') ):
                    X_test_all, Y_test_all = dataset.get_test_data(img_size, 0, 1000)

                    np.save('image_dataset/X_test_all_imagenet_VGG16',X_test_all)
                    np.save('image_dataset/Y_test_all_imagenet_VGG16',Y_test_all)

                else:
                    X_test_all = np.load('image_dataset/X_test_all_imagenet_VGG16.npy')
                    Y_test_all = np.load('image_dataset/Y_test_all_imagenet_VGG16.npy')
                       
                if not (path.exists('image_dataset/X_test_all_imagenet_VGG16_pool') and path.exists('image_dataset/Y_test_all_imagenet_VGG16_pool') ):

                    X_test_all_pool, Y_test_all_pool = dataset.get_test_data(img_size, 0, 5000)
                    np.save('image_dataset/X_test_all_imagenet_VGG16_pool',X_test_all_pool)
                    np.save('image_dataset/Y_test_all_imagenet_VGG16_pool',Y_test_all_pool)
                
                else:
                    X_test_all_pool = np.load('image_dataset/X_test_all_imagenet_VGG16_pool.npy')
                    Y_test_all_pool = np.load('image_dataset/Y_test_all_imagenet_VGG16_pool.npy')

            if FLAGS.model_name == 'resnet50':
                if not (path.exists('image_dataset/X_test_all_imagenet_ResNet50') and path.exists('image_dataset/Y_test_all_imagenet_ResNet50') ):
                    X_test_all, Y_test_all = dataset.get_test_data(img_size, 0, 1000)

                    np.save('image_dataset/X_test_all_imagenet_ResNet50',X_test_all)
                    np.save('image_dataset/Y_test_all_imagenet_ResNet50',Y_test_all)

                else:
                    X_test_all = np.load('image_dataset/X_test_all_imagenet_ResNet50.npy')
                    Y_test_all = np.load('image_dataset/Y_test_all_imagenet_ResNet50.npy')
                       
                if not (path.exists('image_dataset/X_test_all_imagenet_ResNet50_pool') and path.exists('image_dataset/Y_test_all_imagenet_ResNet50_pool') ):

                    X_test_all_pool, Y_test_all_pool = dataset.get_test_data(img_size, 0, 5000)
                    np.save('image_dataset/X_test_all_imagenet_ResNet50_pool',X_test_all_pool)
                    np.save('image_dataset/Y_test_all_imagenet_ResNet50_pool',Y_test_all_pool)
                
                else:
                    X_test_all_pool = np.load('image_dataset/X_test_all_imagenet_ResNet50_pool.npy')
                    Y_test_all_pool = np.load('image_dataset/Y_test_all_imagenet_ResNet50_pool.npy')

    else:
        X_test_all, Y_test_all = dataset.get_test_dataset()
        np.save('image_dataset/X_test_all_cifar',X_test_all)
        np.save('image_dataset/Y_test_all_cifar',Y_test_all)

    
    #if bypass load run this then
    
    
    """
    print ('loading')
    if FLAGS.dataset_name == "CIFAR-10":
        X_test_all = np.load('image_dataset/X_test_all_cifar.npy')
        Y_test_all = np.load('image_dataset/Y_test_all_cifar.npy')
    elif FLAGS.dataset_name == "ImageNet":
        if FLAGS.model_name == 'vgg16':
            X_test_all = np.load('image_dataset/X_test_all_imagenet_VGG16.npy')
            Y_test_all = np.load('image_dataset/Y_test_all_imagenet_VGG16.npy')
    ...
    x = None#tf.placeholder(tf.float32, shape=(None, dataset.image_size, dataset.image_size, dataset.num_channels))
    ...
    print ('saving')
    print (FLAGS.dataset_name,FLAGS.model_name)
    if FLAGS.dataset_name == "CIFAR-10":
        np.save('image_dataset/selected_X_cifar',X_test)
        np.save('image_dataset/selected_Y_cifar',Y_test)
        np.save('image_dataset/selected_GT_cifar',Y_pred)
    elif FLAGS.dataset_name == "ImageNet":
        if FLAGS.model_name == 'vgg16':
            np.save('image_dataset/selected_X_imagenet_VGG16',X_test)
            np.save('image_dataset/selected_Y_imagenet_VGG16',Y_test)
            np.save('image_dataset/selected_GT_imagenet_VGG16',Y_pred)

        if FLAGS.model_name == 'resnet50':
            np.save('image_dataset/selected_X_imagenet_ResNet50',X_test)
            np.save('image_dataset/selected_Y_imagenet_ResNet50',Y_test)
            np.save('image_dataset/selected_GT_imagenet_ResNet50',Y_pred)
    ...
    """select image for adaptive attack
    """
    print ('select candidates for LL')
    ll_index = np.argmax(Y_test_target_ll,axis=1)
    pool_index = np.argmax(Y_test_all_pool,axis=1)
    selected_index_pool=[]
    for index in ll_index:
        selected_from_pool = np.where(pool_index==index)
        #print (selected_from_pool[0][0])
        if len(selected_from_pool[0])==0:
            print ('error: no sample for index', index)
            exit(0)
        selected_index_pool.append(selected_from_pool[0][0]) 
    selected_x_pool_ll = X_test_all_pool[selected_index_pool]

    print ('select candidates for next')
    next_index = np.argmax(Y_test_target_next,axis=1)
    pool_index = np.argmax(Y_test_all_pool,axis=1)
    selected_index_pool=[]
    for index in next_index:
        ...
        if len(selected_from_pool[0])==0:
            ...
            exit(0)
        selected_index_pool.append(selected_from_pool[0][0]) 
    selected_x_pool_next = X_test_all_pool[selected_index_pool]

    del X_test_all_pool
    del Y_test_all_pool
    if FLAGS.dataset_name == "CIFAR-10":
        np.save('image_dataset/Y_test_target_next_cifar',Y_test_target_next)
        np.save('image_dataset/Y_test_target_ll_cifar',Y_test_target_ll)
    elif FLAGS.dataset_name == "ImageNet":
        if FLAGS.model_name == 'vgg16':
            np.save('image_dataset/Y_test_target_next_ImageNet_vgg16',Y_test_target_next)
            np.save('image_dataset/Y_test_target_ll_ImageNet_vgg16',Y_test_target_ll)

        if FLAGS.model_name == 'resnet50':
            np.save('image_dataset/Y_test_target_next_ImageNet_resnet50',Y_test_target_next)
            np.save('image_dataset/Y_test_target_ll_ImageNet_resnet50',Y_test_target_ll)
    ...
    for attack_string in attack_string_list:
        ...
        if 'targeted' in attack_params:
            ...
            if targeted == 'next':
                ...
                selected_x_pool = selected_x_pool_next
            elif targeted == 'll':
                Y_test_target = Y_test_target_ll
                selected_x_pool = selected_x_pool_ll
            elif targeted == False:
                attack_params['targeted'] = False
                Y_test_target = Y_test.copy()
                selected_x_pool = selected_x_pool_next
            np.save('Y_test_target',Y_test_target)
        else:
            targeted = False
            attack_params['targeted'] = False
            Y_test_target = Y_test.copy()
            selected_x_pool=None

        x_adv_fname = "%s_%s.pickle" % (task_id, attack_string)
        x_adv_fpath = os.path.join(X_adv_cache_folder, x_adv_fname)
        duration=0
        
        X_test_adv, aux_info = maybe_generate_adv_examples(sess, model, selected_x_pool, y, X_test, Y_test_target, attack_name, attack_params, use_cache = x_adv_fpath, verbose=FLAGS.verbose, attack_log_fpath=attack_log_fpath)
        ...
        X_test_adv_discret = X_test_adv #reduce_precision_py(X_test_adv, 256)
    ...
    fieldnames = ['dataset_name', 'model_name', 'attack_string', 'duration_per_sample', 'discretization', 'success_rate', 'mean_confidence', 'mean_l2_dist', 'mean_li_dist', 'mean_l0_dist_value', 'mean_l0_dist_pixel','mean_l1_dist']
